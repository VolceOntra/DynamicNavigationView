...
struct ContentView: View {
    ...
    let vm: DNViewModel = .init(root: .init(width: 0, height: 0))

    func examples() -> [DNDefination] {
        [
            ...
            DNDefination(width: 220, height: 70, mainAccessory: AnyView(HStack {
                Image("cover")
                    ...
                    .offset(x: -5)
                VStack(alignment: .leading) {
                    Text("Entropy").bold().foregroundColor(colorScheme == .dark ? .black : .white)
            ...
            // Phone ring status
            DNDefination(width: 125, height: 50, mainAccessory: AnyView(HStack {
            ...
            DNDefination(width: 200, height: 200, mainAccessory: AnyView(Image(systemName: "faceid")
                .resizable()
                .aspectRatio(contentMode: .fit)
                .padding().foregroundColor(colorScheme == .dark ? .black : .white))),
            // Charging Status
            DNDefination(width: 320, height: 50, mainAccessory: AnyView(
                HStack() {
                    Text("Charging").bold().foregroundColor(colorScheme == .dark ? .black : .white)
                    Spacer()
